 ...
 * imports this directly except the mount that wires that getter and the host app's
 * components.
 *
 * The host app for this build is the Calculator (`app/calc`), so the shape below is
 * the calculator's real state: the operand being typed, the stored accumulator, the
 * pending operator, and the small flags that drive entry. Keeping it here (not a
 * second store) is what makes the "exactly one store" rule meaningful and what makes
 * the `clientState` artifact show the calculator's live state at report time.
 */
...
/** The mandated store hook - the Calculator's state machine. */
 ...
 * dropped - only the calculator's data fields are captured, so the agent sees the
 * exact operand/accumulator/operator at report time.
 */
export function getState(): Record<string, unknown> {
  const { current, accumulator, operator, overwrite, error } =
    usePaikkoStore.getState();
